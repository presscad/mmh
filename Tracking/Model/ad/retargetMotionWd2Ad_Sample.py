...
def retargetMotion():
    ...
    '''
    for pose in sMotion:
        pose.setRootPos(pose.getJointPositionGlobal(0) + (0,0,-0.5))

    for pose in tMotion:
        pose.setRootPos(pose.getJointPositionGlobal(0) + (0,0.0,0.5))
    '''

    showMotion([sMotion, tMotion], ["wd2", "ad"], [(255,25,25),(0,255,0)])
    #showMotion([sMotion, tMotion, rMotion], ["wd2", "ad", "written_ad"], [(255,25,25),(0,255,0),(255,255,0)])
...
def showMotion(motions, names=None, colors=None):
    ...
    for i in range(len(motions)):
        ...
        m1 = motions[i][90:].copy()
        m2 = motions[i][120:].copy()
        m3 = motions[i][145:].copy()
        viewer.doc.addRenderer(name+"m1", yr.JointMotionRenderer(m1,color, yr.POLYGON_LINE))
        viewer.doc.addObject(name+"m1", m1)
        viewer.doc.addRenderer(name+"m2", yr.JointMotionRenderer(m2,color, yr.POLYGON_LINE))
        viewer.doc.addObject(name+"m2", m2)
        viewer.doc.addRenderer(name+"m3", yr.JointMotionRenderer(m3,color, yr.POLYGON_LINE))
        viewer.doc.addObject(name+"m3", m3)
