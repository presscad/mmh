from fltk import *
import copy, os.path, cPickle, time, math
import numpy as np
...
def walkings():
    ...
    # global parameters
    # Kt, Dt for PD control
    # Ks, Ds for penalty force
    Kt = 20.;       Dt = 2*(Kt**.5)
    Ks = 4000.;    Ds = 2*(Ks**.5)
    #Ks = 2000.;    Ds = 2*(Ks**.5) # ys examples
    mu = 1.
...
#    c_swf_offset = .005
    ...
    # coefficients of SWING_FOOT_HEIGHT
#    c5 = .5;    c6 = .01
    c5 = .5;    c6 = .02
#    c5 = .5;    c6 = .05
#    c5 = 1.;    c6 = .05
#    c5 = .0;    c6 = .0
    ...
    # coefficients of STANCE_FOOT_BALANCING
    K_stb_vel = .1
    K_stb_pos = .1
...
#   dcjo test
#    K_swp_vel_sag = .15; K_swp_vel_cor = .29; K_swp_pos_sag = .6; K_swp_pos_cor = .2

#   3 parameter
#    K_swp_vel_sag = .2; K_swp_vel_cor = .286; K_swp_pos_sag = K_swp_vel_sag; K_swp_pos_cor = K_swp_vel_cor
#    K_swp_pos_sag_faster = K_swp_vel_sag
#    K_stb_vel = 0.1
#    K_stb_pos = K_stb_vel
#    K_swp_vel_sag = .24; K_swp_vel_cor = .284; K_swp_pos_sag = K_swp_vel_sag; K_swp_pos_cor = K_swp_vel_cor
    '''
    K_swp_vel_sag = 0.; K_swp_vel_cor = 0.; K_swp_pos_sag = K_swp_vel_sag; K_swp_pos_cor = K_swp_vel_cor
    K_swp_pos_sag_faster = K_swp_vel_sag/3.
    K_stb_vel = 0.4535
    K_stb_pos = K_stb_vel

    filename = 'ad_WalkForwardNormal00.bvh'
    '''
...
#    filename = 'wd2_WalkAzuma01_REPEATED.bvh'   # 2 frame diff
    ...
    mu = 2.
    K_spc = 1.5
    K_swp_vel_sag = .1; K_swp_vel_cor = 0.2; K_swp_pos_sag = .2; K_swp_pos_cor = .5
#    K_swp_vel_sag = .1; K_swp_vel_cor = 0.2; K_swp_pos_sag = .1; K_swp_pos_cor = .5
    K_swp_pos_sag_faster = .0
    K_swp_vel_sag = .0; K_swp_vel_cor = 0.2; K_swp_pos_sag = .0; K_swp_pos_cor = .2
    K_swp_pos_sag_faster = .0
    K_stb_vel = 0.3
    K_stb_pos = K_stb_vel

    filename = 'ad_WalkAzuma01.bvh'
...
#    filename = 'wd2_WalkBackward00_REPEATED.bvh'
    ...
    STANCE_FOOT_PUSH =          False#True
    STANCE_FOOT_BALANCING =     True
   
    STOP_PHASE_CONTROL = True
...
#   motionModel.recordVelByFiniteDiff()
    ...
    print
    #print "dof", motion_ori[0].skeleton.getTotalDOF()
    #print "joint", motion_ori[0].skeleton.getJointNum()
    ...
    # dcjo
    motion_stop_phase_control = ym.JointMotion()
    ...
    motion_simulated = ym.JointMotion()
...
#    stf_push_func = [yfg.zero]
    ...
    lID = controlModel.name2id('ankle_l');      rID = controlModel.name2id('ankle_r')
    lUpLeg = skeleton.getJointIndex('hip_l');   rUpLeg = skeleton.getJointIndex('hip_r')
    lKnee = skeleton.getJointIndex('knee_l');   rKnee = skeleton.getJointIndex('knee_r')
    lFoot = skeleton.getJointIndex('ankle_l');  rFoot = skeleton.getJointIndex('ankle_r')
    spine = skeleton.getJointIndex('back')
    
    uppers = [skeleton.getJointIndex(name) for name in ['ground_pelvis', 'back', 'shoulder_l', 'elbow_l', 'radioulnar_l', 'radius_hand_l', 'shoulder_r', 'elbow_r', 'radioulnar_r', 'radius_hand_r']]
    upperMass = sum([bodyMasses[i] for i in uppers])
    lLegs = [skeleton.getJointIndex(name) for name in ['hip_l', 'knee_l', 'ankle_l', 'mtp_l']]
    rLegs = [skeleton.getJointIndex(name) for name in ['hip_r', 'knee_r', 'ankle_r', 'mtp_r']]
    allJoints = set(range(skeleton.getJointNum()))
...
#        viewer.doc.addRenderer('motion_control', yr.JointMotionRenderer(motion_control, (255,0,0), yr.LINK_BONE))
        viewer.doc.addRenderer('motion_simulated', yr.JointMotionRenderer(motion_simulated, (255,255,255), yr.LINK_BONE))
...
#        rd_vec1[0] = diff_dCM_sag; rd_vecori1[0] = CM_tar
        #diff_vels_sag.append(mm.length(diff_dCM_sag))
        diff_vels_sag.append(mm.length(diff_dCM_sag) if np.inner(direction, diff_dCM_sag) > 0 else -mm.length(diff_dCM_sag))
        diff_vels_cor.append(mm.length(diff_dCM_cor))
        ...
        diff_ds_sag.append(mm.length(diff_CMr_sag))
        diff_ds_cor.append(mm.length(diff_CMr_cor))
        diff_CMr_sag_axis = np.cross((0,1,0), diff_CMr_sag)
        diff_CMr_cor_axis = np.cross((0,1,0), diff_CMr_cor)
       
        print "diff_vels_sag", sum(diff_vels_sag)/len(diff_vels_sag)
#        print "diff_vels_cor", sum(diff_vels_cor)/len(diff_vels_cor)
#        print "diff_ds_sag", sum(diff_ds_sag)/len(diff_ds_sag)
#        print "diff_ds_cor", sum(diff_ds_cor)/len(diff_ds_cor)

        # when frame = curInterval[0], t = 0
        # when frame = curInterval[1], t = 1
...
#                    motion_match_stl[frame].mulJointOrientationGlobal(stanceLeg, mm.scaleSO3(R_y, t_y))
        ...
        # dcjo STOP_PHASE_CONTROL
        motion_stop_phase_control.append(motion_match_stl[frame].copy())
        motion_stop_phase_control.goToFrame(frame)
        if STOP_PHASE_CONTROL:
            if curState == yba.GaitState.STOP:
                y_ori = np.dot(controlModel.getJointOrientationGlobal(0), np.array([0.,1.,0.],float))
                y_ori_tar = np.dot(motion_seg[frame].getJointOrientationGlobal(0), np.array([0.,1.,0.],float))
                rotV = np.cross(y_ori, y_ori_tar)
                rotA = mm.ACOS(np.inner(y_ori, y_ori_tar)) * 180/math.pi
                R_spc = mm.exp(rotV * rotA * K_spc * -1.0)
                
                for stanceFoot in stanceFoots:
                    motion_stop_phase_control[frame].mulJointOrientationGlobal(stanceFoot, R_spc)

        # swing foot placement
        #motion_swf_placement.append(motion_match_stl[frame].copy())
        motion_swf_placement.append(motion_stop_phase_control[frame].copy())
        motion_swf_placement.goToFrame(frame)
        if SWING_FOOT_PLACEMENT:
            # t_swing_foot_placement : hermite2nd, 0 -> 1
...
#                    R_zero = 
                # If the previous step_length is longer than the step_length of reference motion,
                # a stance foot push feed back rule makes the biped slower
                # If not, makes the biped faster
...
#            rd_vecori1[0]= controlModel.getJointPositionGlobal(stanceFoots[0])
        ...
        # record simution result
        motion_simulated.append(motion_control[frame].copy())
        ...
        if SEGMENT_EDITING:
            if curState==yba.GaitState.STOP:
                ...
                # check whether a swing foot contacts to something or not
                contact = False
                ...
                if contact:
                    # early contact : acc_offset[0] < 0
                    # contact in time : acc_offset[0] == 0
...
#                motion_stitch.extend(ymb.getStitchedNextMotion(curSeg, motion_control[-1], transitionLength, stitch_func, True, False))
                ...
                if NO_FOOT_SLIDING:
                    # the segment right in front of last segment
                ...
                motion_stitch.extend(ymb.getStitchedNextMotion(curSeg, d, transitionLength, stitch_func, True, True))
