...
# dcjo
    '''
    Ks = 4000.;    Ds = 2*(Ks**.5)
    # K_stb func : hermite2nd
    K_swp_vel_sag = .0; K_swp_vel_cor = .30; K_swp_pos_sag = K_swp_vel_sag; K_swp_pos_cor = K_swp_vel_cor
    K_swp_pos_sag_faster = K_swp_vel_sag
    K_stb_vel = 0.10364502220998
    K_stb_vel = 0.1036
    K_stb_pos = K_stb_vel
    filename = 'wd2_WalkForwardNormal00.bvh'
...
# dcjo
    '''
    Ks = 4000.;    Ds = 2*(Ks**.5)
    K_swp_vel_sag = .0; K_swp_vel_cor = .2; K_swp_pos_sag = K_swp_vel_sag; K_swp_pos_cor = K_swp_vel_cor
    K_swp_pos_sag_faster = K_swp_vel_sag
    K_stb_vel = 0.05
    K_stb_pos = K_stb_vel
    filename = 'wd2_WalkSoldier00.bvh'    # K_swp_pos_sag = .0
    ...
    '''
    # combine swp sagital plane control and coronal plane control
    # use K_swp_sag as K_swp
    K_swp_vel_sag = .4; K_swp_vel_cor = .0; K_swp_pos_sag = K_swp_vel_sag ; K_swp_pos_cor = K_swp_vel_cor
    K_swp_pos_sag_faster = K_swp_vel_sag
    K_stb_vel = K_swp_vel_sag * 0.2
    K_stb_pos = K_stb_vel
    filename = 'wd2_WalkForwardNormal00.bvh'
    '''

    # F, N, NR
    K_swp_vel_sag = .0; K_swp_vel_cor = .2; K_swp_pos_sag = .4; K_swp_pos_cor = 0.2
    K_swp_pos_sag_faster = 0.0#K_swp_vel_sag
    K_stb_vel = 0.1
    K_stb_pos = K_stb_vel
    ...
    #filename = 'wd2_WalkForwardNormal00.bvh'
    #filename = 'wd2_WalkForwardNormal00_REPEATED.bvh'
    K_swp_vel_sag = .3375; K_swp_vel_cor = .0; K_swp_pos_sag = .3375; K_swp_pos_cor = 0.15
    K_stb_vel = 0.0
    K_stb_pos = K_stb_vel
...
##    filename = 'wd2_WalkSameSame01_REPEATED.bvh'
    ...
    '''
    K_swp_vel_sag = .15; K_swp_vel_cor = .4; K_swp_pos_sag = .15; K_swp_pos_cor = .4
    K_swp_pos_sag_faster = 0.0#K_swp_vel_sag
    K_stb_vel = 0.0
    K_stb_pos = K_stb_vel
    filename = 'wd2_WalkForwardVFast00.bvh'
    ...
    '''
    K_swp_vel_sag = .30; K_swp_vel_cor = .40; K_swp_pos_sag = .30; K_swp_pos_cor = .40;
    K_swp_vel_sag = .26; K_swp_vel_cor = .30; K_swp_pos_sag = .26; K_swp_pos_cor = .35;
    K_swp_vel_sag = .26; K_swp_vel_cor = .20; K_swp_pos_sag = .26; K_swp_pos_cor = 0.65;
    K_swp_pos_sag_faster = 0.0#K_swp_vel_sag
    K_stb_vel = 0.0
    K_stb_pos = K_stb_vel
    filename = 'wd2_WalkForwardVFast00_REPEATED.bvh'
#    filename = 'wd2_WalkForwardVFast00.bvh'
    ...
    '''    
    K_swp_vel_sag = .0; K_swp_vel_cor = .20; K_swp_pos_sag = K_swp_vel_sag; K_swp_pos_cor = K_swp_vel_cor
    K_swp_pos_sag_faster = K_swp_vel_sag
    K_stb_vel = 0.3
    K_stb_pos = K_stb_vel
    filename = 'wd2_WalkAzuma01.bvh'
...
#    filename = 'wd2_WalkBackward00_REPEATED.bvh'
    ...
    # dcjo stance foot, swing foot push control
    STANCE_FOOT_CONTROL =     False#True
    SWING_FOOT_CONTROL =     False#True
    FOOT_PUSH = False
    if FOOT_PUSH:
        STANCE_FOOT_BALANCING =     False
        STANCE_FOOT_CONTROL =     True
        SWING_FOOT_CONTROL =     True
...
#    stf_balancing_func = yfg.concatenate([yfg.hermite2nd, yfg.one], [c_landing_duration])
    ...
    # dcjo
    #stf_balancing_func = yfg.sine
    stf_control_func = yfg.hermite2nd
    swf_control_func = yfg.concatenate([lambda x : 1. - yfg.hermite2nd(x), yfg.zero], [c_landing_duration])
...
#   motionModel.recordVelByFiniteDiff()
    ...
    # dcjo stance foot, swing foot push control
    motion_stf_control = ym.JointMotion()
    motion_swf_control = ym.JointMotion()
...
#        viewer.doc.addRenderer('motion_stf_push', yr.JointMotionRenderer(motion_stf_push, (50,255,200), yr.LINK_BONE))
    #    viewer.doc.addRenderer('motion_stf_balancing', yr.JointMotionRenderer(motion_stf_balancing, (255,100,255), yr.LINK_BONE))
        ...
        viewer.doc.addRenderer('motion_stf_control', yr.JointMotionRenderer(motion_stf_control, (200,0,255), yr.LINK_BONE))
        viewer.doc.addRenderer('motion_swf_control', yr.JointMotionRenderer(motion_swf_control, (100,0,255), yr.LINK_BONE))
...
#        CM_tar = yrp.getCM(motion_seg.getJointPositionsGlobal(prev_frame), bodyMasses, totalMass)
        ...
        #dcjo test
#        dCM_tar = motion_stitch.getJointVelocityGlobal(0, prev_frame)
#        CM_tar = motion_stitch.getJointPositionGlobal(0, prev_frame)
#        stf_tar = motion_stitch.getJointPositionGlobal(stanceFoots[0], prev_frame)
...
#                    R_zero = 
        ...
        # dcjo
        motion_stf_control.append(motion_stf_balancing[frame].copy())
        motion_stf_control.goToFrame(frame)
        if STANCE_FOOT_CONTROL:
            '''
            if np.dot(mm.projectionOnPlane(CMr,(1,0,0),(0,0,1)), direction) < 0:
                R_stf = mm.exp(diff_dCM_axis * K_stf_ctl * stf_control_func(t))
                R_stf = np.dot(R_stf, mm.exp(diff_CMr_axis * K_stf_ctl * stf_control_func(t)))
            else:
                R_stf = mm.exp(diff_dCM_axis * -K_stf_ctl * stf_control_func(t))
                R_stf = np.dot(R_stf, mm.exp(diff_CMr_axis * -K_stf_ctl * stf_control_func(t)))

            for stanceFoot in stanceFoots:
                motion_stf_control[frame].mulJointOrientationGlobal(stanceFoot, R_stf)
            '''
            stf_control_func0 = yfg.concatenate([yfg.sine, yfg.zero],[0.6])
            stf_control_func1 = yfg.concatenate([yfg.zero, yfg.hermite2nd], [0.6])

            if t < 0.6:
                R_stf = mm.exp(diff_dCM_axis * K_stf_ctl * stf_control_func0(t))
                R_stf = np.dot(R_stf, mm.exp(diff_CMr_axis * K_stf_ctl * stf_control_func0(t)))
            else:
                R_stf = mm.exp(diff_dCM_axis * -K_stf_ctl * stf_control_func1(t))
                R_stf = np.dot(R_stf, mm.exp(diff_CMr_axis * -K_stf_ctl * stf_control_func1(t)))

            for stanceFoot in stanceFoots:
                motion_stf_control[frame].mulJointOrientationGlobal(stanceFoot, R_stf)

        motion_swf_control.append(motion_stf_control[frame].copy())
        motion_swf_control.goToFrame(frame)
        if SWING_FOOT_CONTROL:
            R_swc = mm.exp(diff_dCM_axis * -K_swf_ctl * swf_control_func(t))
            R_swc = np.dot(R_swc, mm.exp(diff_CMr_axis * -K_swf_ctl * swf_control_func(t)))
            for swingFoot in swingFoots:
                motion_swf_control[frame].mulJointOrientationGlobal(swingFoot, R_swc)

        # control trajectory
#        motion_control.append(motion_stf_balancing[frame].copy())
        motion_control.append(motion_swf_control[frame].copy())
